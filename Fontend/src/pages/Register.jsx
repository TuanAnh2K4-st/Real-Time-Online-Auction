...
export default function Register() {
  // Component con: hiển thị trạng thái từng quy tắc mật khẩu (đã đạt / chưa)
  // Props: `isValid` (boolean) - có đạt quy tắc không, `text` - mô tả quy tắc
  const RuleIndicator = ({ isValid, text }) => (
    <div className={`flex items-center gap-2 transition-all duration-300 ${isValid ? 'text-emerald-400' : 'text-slate-500'}`}>
      {isValid ? <CheckCircle2 size={14} className="flex-shrink-0" /> : <div className="w-3.5 h-3.5 rounded-full border border-slate-700 flex-shrink-0" />}
    ...
    // `formData`: lưu giá trị input của form chung (username/companyName/room/email/password)
    ...
    // `confirmPassword`: trường nhập lại mật khẩu để đối chiếu
    const [confirmPassword, setConfirmPassword] = useState('');

    // Các state điều khiển hiển thị mật khẩu, loading, validation và thông báo
    ...
    // Các quy tắc kiểm tra mật khẩu: dùng để hiển thị feedback theo thời gian thực
    // - `length`: ít nhất 12 ký tự
    // - `uppercase`, `lowercase`, `number`, `special`: các loại ký tự yêu cầu
    const passwordRules = {
    ...
    // Hiển thị Toast Notification tạm thời để báo lỗi / thành công / cảnh báo
    ...
    // Cập nhật `formData` khi người dùng nhập input
    // Sử dụng `name` của input để gán vào đúng trường trong `formData`
    ...
    // - Kiểm tra các điều kiện client-side trước khi gọi API
    // - Gọi `registerUserApi` hoặc `registerSellerApi` tuỳ `type`
    // - Hiển thị thông báo và chuyển hướng khi đăng ký thành công
    const handleSubmit = async (e) => {
      e.preventDefault();
      // Đánh dấu đã submit để bật các hiệu ứng validaton (ví dụ: viền đỏ)
      setIsSubmitted(true);

      // Kiểm tra mật khẩu, xác nhận mật khẩu và đồng ý điều khoản
      if (!isPasswordValid) {
        showNotification("Mật khẩu chưa đạt yêu cầu bảo mật!", "warning");
        return;
      }
      if (!doPasswordsMatch) {
      ...
      if (!agreed) {
        showNotification("Bạn cần đồng ý với điều khoản dịch vụ.", "warning");
        return;
      }
      ...
      try {
        if (type === 'user') {
          // Chuẩn bị payload cho API đăng ký người dùng cá nhân
          const payload = {
        ...
        // Nếu API trả về thành công, hiển thị thông báo rồi chuyển hướng tới trang đăng nhập
        showNotification('Đăng ký thành công! Chuyển hướng đến trang đăng nhập...', 'success');
        setTimeout(() => navigate('/login'), 900);
      } catch (err) {
        // Thử lấy thông điệp lỗi từ response, fallback sang stringify
        const message = err?.message || err?.error || JSON.stringify(err) || 'Lỗi trong quá trình đăng ký';
          ...
          <div className="flex-1 p-8 md:p-12 bg-slate-900/40 relative overflow-y-auto max-h-[90vh] md:max-h-[850px]">
            <div className="max-w-md mx-auto">
              ...
              <div className="flex bg-slate-800/50 p-1.5 rounded-2xl mb-8 border border-white/5 backdrop-blur-md">
                <button
                  type="button"
                  className={`flex-1 flex justify-center items-center py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 ${
                    type === 'user' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200'
                  }`}
                ...
                <button
                  type="button"
                  className={`flex-1 flex justify-center items-center py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 ${
                    type === 'seller' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200'
                  }`}
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="••••••••••••"
                      className={`w-full pl-12 pr-12 py-3.5 bg-slate-800/30 border rounded-2xl focus:outline-none focus:ring-2 transition-all text-white
                        ${isSubmitted && !isPasswordValid ? 'border-rose-500/50 focus:ring-rose-500/20' : 'border-slate-700/50 focus:ring-blue-500/40'}
                      `}
                    />
                    <button
                      type="button"
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="••••••••••••"
                      className={`w-full pl-12 pr-12 py-3.5 bg-slate-800/30 border rounded-2xl focus:outline-none focus:ring-2 transition-all text-white
                        ${confirmPassword.length > 0 && !doPasswordsMatch ? 'border-rose-500/50 focus:ring-rose-500/20' : 
                          doPasswordsMatch ? 'border-emerald-500/50 focus:ring-emerald-500/20' : 'border-slate-700/50 focus:ring-blue-500/40'}
                      `}
                    />
                    <button
                      type="button"
                  ...
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <div className="flex-shrink-0 mt-0.5 relative">
                      ...
                      <div className={`w-5 h-5 rounded-lg border flex items-center justify-center transition-all 
                        ${agreed ? 'bg-blue-600 border-blue-600' : 'bg-slate-800 border-slate-700 group-hover:border-slate-500'}
                      `}>
