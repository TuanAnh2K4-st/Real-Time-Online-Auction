        ...
        <div className={`flex items-center transition-all duration-300 ${isExpanded ? 'opacity-100 pr-2' : 'opacity-0 w-0'}`}>
          {isExpanded && (
            <div className="flex items-center gap-1">
               {searchValue ? (
                 <button 
                  onClick={(e) => { e.stopPropagation(); handleClearOrClose(); }} 
...
// BẢN NAVBAR MỚI CẬP NHẬT
const Navbar = () => {
  const [scrolled, setScrolled] = useState(false);
  
  // Trạng thái giả lập đăng nhập
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  
  // Trạng thái cho các dropdown
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          {['Trang chủ', 'Đấu Live', 'Lịch sử', 'Hướng dẫn'].map((item, i) => (
            <a 
              ...
              className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${
                i === 0 ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:text-white hover:bg-white/5'
              }`}
            >
              {item}
            </a>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          {!isLoggedIn ? (
            ...
            <button 
              onClick={() => setIsLoggedIn(true)} // Giả lập Đăng nhập
              className="flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-2xl font-bold hover:scale-105 active:scale-95 transition-all shadow-xl shadow-blue-900/40"
            >
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button className="relative p-3 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-colors">
                ...
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-slate-950">
                  3
                </span>
              </button>
              ...
              <div className="relative" ref={profileRef}>
                <button 
                  ...
                  className="relative p-0.5 rounded-2xl bg-gradient-to-tr from-blue-600 to-cyan-400 hover:scale-105 transition-transform"
                >
                  <img 
                    src={MOCK_USER.avatar} 
                    alt="User" 
                    className="w-10 h-10 sm:w-11 sm:h-11 rounded-[14px] border-2 border-slate-950 object-cover"
                  />
                </button>
                ...
                {isProfileOpen && (
                  <div className="absolute top-full right-0 mt-3 w-64 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200 p-2">
                    <div className="p-4 bg-white/5 rounded-2xl mb-2 flex items-center gap-3">
                      <img src={MOCK_USER.avatar} alt="User" className="w-12 h-12 rounded-xl object-cover" />
                      <div>
                        <p className="text-white font-bold">{MOCK_USER.name}</p>
                        <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{MOCK_USER.level}</p>
                      </div>
                    </div>
                    <div className="flex flex-col gap-1 p-1">
                      <button className="flex items-center gap-3 p-3 rounded-xl text-sm font-bold text-slate-300 hover:bg-white/5 hover:text-white transition-colors w-full text-left">
                        <User className="w-4 h-4 text-slate-400" /> Hồ sơ cá nhân
                      </button>
                      <button 
                        onClick={() => { setIsLoggedIn(false); setIsProfileOpen(false); }} // Giả lập Đăng xuất
                        className="flex items-center gap-3 p-3 rounded-xl text-sm font-bold text-red-400 hover:bg-red-500/10 transition-colors w-full text-left mt-1 border-t border-white/5"
                      >
              ...
              <div className="relative" ref={menuRef}>
                ...
                {isUserMenuOpen && (
                  <div className="absolute top-full right-0 mt-3 w-72 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden z-50 animate-in slide-in-from-top-2 duration-200 p-2">
                    ...
                    <div className="flex flex-col gap-1">
                      {[
                        { icon: <Wallet className="w-5 h-5" />, label: "Ví thanh toán", value: formatCurrency(MOCK_USER.balance), color: "text-green-400", bg: "bg-green-400/10" },
                        ...
                        <button key={idx} className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 transition-colors group w-full text-left">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
        <div className="lg:col-span-7 space-y-8">
          <div className="inline-flex items-center gap-3 px-4 py-2 bg-white/5 rounded-full border border-white/10 backdrop-blur-sm">
            ...
            <span className="text-xs font-black text-blue-400 uppercase tracking-widest">Sàn đấu giá số 1 Việt Nam</span>
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-xl font-medium leading-relaxed">
            Hệ thống đấu giá thời gian thực với công nghệ bảo mật tiên tiến. Nơi những giá trị đích thực được tôn vinh và bảo vệ pháp lý nghiêm ngặt.
          </p>

          <div className="flex flex-wrap gap-5">
            <button className="px-10 py-5 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-2xl shadow-blue-900/50 hover:bg-blue-500 hover:-translate-y-1 transition-all flex items-center gap-3 group">
              Khám phá ngay
              <ArrowRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
            </button>
            <button className="px-10 py-5 bg-white/5 text-white border border-white/10 rounded-2xl font-black text-lg hover:bg-white/10 transition-all flex items-center gap-3 group">
              <div className="relative">
                    ...
                    {HERO_IMAGES.map((_, idx) => (
                      <div 
                        ...
                        className={`h-1.5 rounded-full transition-all duration-500 ${
                          idx === currentImg ? 'w-8 bg-blue-500' : 'w-2 bg-white/30'
                        }`}
                      />
                    ))}
                  </div>
...
const SellerSection = () => (
  <section className="max-w-7xl mx-auto px-6 py-12">
    <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-4 text-center md:text-left">
      ...
      <div className="flex items-center gap-2 text-blue-400 font-bold text-sm cursor-pointer hover:underline">
        Trở thành Seller <ArrowRight className="w-4 h-4" />
      </div>
    </div>
      ...
      {TOP_SELLERS.map((seller) => (
        <div key={seller.id} className="group relative bg-slate-900/40 rounded-3xl p-6 border border-white/5 hover:border-blue-500/50 hover:bg-slate-900/80 transition-all duration-500 overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <Verified className="w-24 h-24 text-blue-500 rotate-12" />
          </div>

          <div className="flex items-center gap-5 relative z-10">
            <div className="relative">
              <div className="absolute -inset-1 bg-gradient-to-tr from-blue-600 to-cyan-400 rounded-full blur opacity-40 group-hover:opacity-100 transition-opacity"></div>
              <img src={seller.avatar} alt={seller.name} className="relative w-16 h-16 rounded-full object-cover border-2 border-slate-950" />
            ...
            <div className="space-y-1">
              <h4 className="text-white font-black text-lg group-hover:text-blue-400 transition-colors">{seller.name}</h4>
              <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider">{seller.category}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-white/5 relative z-10">
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Thành công</p>
              <div className="flex items-center gap-2">
                <Zap className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                <span className="text-lg font-black text-white">{seller.successSales}</span>
              </div>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Đánh giá</p>
              <div className="flex items-center gap-2">
                <Star className="w-4 h-4 text-blue-400 fill-blue-400" />
                <span className="text-lg font-black text-white">{seller.rating}</span>
              </div>
            </div>
          </div>
        </div>
...
const LiveAuctionCard = ({ session }) => (
  <div className="relative group bg-slate-900/50 rounded-[2.5rem] p-4 border border-white/5 shadow-2xl hover:border-blue-500/30 transition-all duration-500">
    <div className="relative h-80 rounded-[2rem] overflow-hidden mb-6">
      ...
      <div className="absolute top-5 left-5 flex items-center gap-2 bg-red-500 text-white px-4 py-2 rounded-full text-[10px] font-black tracking-widest shadow-lg shadow-red-900/40">
        <span className="w-2 h-2 bg-white rounded-full animate-ping"></span> 
        {session.isLive ? 'LIVE NOW' : session.time}
      </div>
      ...
      <div className="absolute top-5 right-5 bg-white/10 backdrop-blur-md text-white px-4 py-2 rounded-2xl text-[10px] font-bold flex items-center gap-2 border border-white/10">
        <Eye className="w-4 h-4 text-blue-400" /> {session.viewers.toLocaleString()}
      </div>

      <div className="absolute bottom-8 left-8 right-8">
        <h3 className="text-2xl font-black text-white leading-tight mb-2 group-hover:text-blue-400 transition-colors">
          {session.title}
        </h3>
        <p className="text-slate-400 text-sm line-clamp-1">{session.description}</p>
      </div>
    </div>
    ...
    <div className="flex flex-col gap-4 px-2 pb-2">
      <div className="flex items-center justify-between p-4 bg-white/5 rounded-3xl border border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Danh mục sản phẩm</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button className="py-4 bg-blue-600 text-white rounded-2xl font-black text-sm hover:bg-blue-500 transition-all shadow-lg shadow-blue-900/20 flex items-center justify-center gap-2 group">
          Vào phòng <Gavel className="w-4 h-4 group-hover:rotate-12 transition-transform" />
        </button>
        <button className="py-4 bg-white/5 text-white rounded-2xl font-black text-sm hover:bg-white/10 border border-white/10 transition-all flex items-center justify-center gap-2">
          Xem danh sách <ChevronRight className="w-4 h-4 text-slate-500" />
        </button>
      </div>
    </div>
  </div>
...
const ProductCard = ({ item }) => (
  <div className="group bg-slate-900/40 rounded-[2rem] border border-white/5 p-3 hover:border-blue-500/50 hover:bg-slate-900/80 transition-all duration-500 flex flex-col h-full">
    <div className="relative aspect-square rounded-[1.5rem] overflow-hidden bg-slate-800 mb-4">
      <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90 group-hover:opacity-100" />
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 transition-all">
        <button className="p-2.5 bg-slate-950/80 backdrop-blur-md rounded-xl text-white hover:text-red-500 border border-white/10"><Heart className="w-4 h-4" /></button>
        <button className="p-2.5 bg-slate-950/80 backdrop-blur-md rounded-xl text-white hover:text-blue-500 border border-white/10"><Share2 className="w-4 h-4" /></button>
      </div>

      <div className="absolute bottom-4 left-4 right-4 bg-slate-950/80 backdrop-blur-xl py-3 px-4 rounded-2xl flex items-center justify-between border border-white/10">
        <div className="flex items-center gap-2 text-blue-400 font-black text-xs uppercase tracking-tighter italic">
    ...
    <div className="px-2 space-y-4 flex-grow">
      <div>
        <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest block mb-1">{item.category}</span>
        <h3 className="font-bold text-white line-clamp-2 leading-tight text-base">{item.title}</h3>
      </div>

      <div className="flex items-center justify-between pt-4 border-t border-white/5">
        <div>
          <p className="text-[9px] font-bold text-slate-500 uppercase">Giá cao nhất</p>
          <p className="text-lg font-black text-blue-400">{formatCurrency(item.currentBid)}</p>
        </div>
        <button className="bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-500 transition-all shadow-lg shadow-blue-900/50">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="space-y-4">
              ...
              <h2 className="text-4xl md:text-5xl font-black text-white italic tracking-tighter">
                PHIÊN ĐẤU <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400 underline decoration-blue-500/30 underline-offset-8">LIVE CAO CẤP</span>
              </h2>
            </div>
            <button className="group flex items-center gap-2 text-sm font-black text-slate-400 hover:text-white transition-colors">
              Xem lịch phát sóng <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {LIVE_SESSIONS.map(session => (
              <LiveAuctionCard key={session.id} session={session} />
            ))}
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-8">
              <div className="text-center md:text-left space-y-2">
                <h2 className="text-3xl font-black text-white tracking-tighter uppercase">Sàn đấu phổ thông</h2>
                <p className="text-slate-500 font-medium">Hàng ngàn vật phẩm đang chờ đợi bạn gõ búa.</p>
              </div>
              <div className="flex gap-2 p-1 bg-white/5 rounded-2xl border border-white/5">
                {['Mới nhất', 'Giá cao', 'Sắp kết thúc'].map((sort, i) => (
                  <button key={sort} className={`px-5 py-2 rounded-xl text-xs font-bold transition-all ${i === 0 ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}>
                    {sort}
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {NORMAL_AUCTIONS.map(item => (
                <ProductCard key={item.id} item={item} />
              ))}
            </div>
            
            <div className="mt-16 text-center">
              <button className="px-12 py-5 bg-transparent border-2 border-white/10 rounded-2xl font-black text-sm uppercase tracking-widest text-white hover:bg-white hover:text-slate-950 transition-all duration-500">
                Tải thêm sản phẩm
              </button>
            </div>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="p-10 bg-gradient-to-b from-white/5 to-transparent rounded-[2.5rem] border border-white/5 text-center group hover:-translate-y-2 transition-all duration-500">
