...
import {
  ...
  UserPlus, Shield, UserCheck, UserX, Trash2,
  Eye, Mail, Phone, Briefcase, MapPin, Info,
  AlertTriangle, Crown, Store, Truck, User
} from 'lucide-react';
...
const StatusBadge = ({ status, onClick }) => {
  ...
  return (
    <button 
      onClick={onClick}
      className={`px-3 py-1 rounded-full text-[10px] font-black border transition-all cursor-pointer select-none active:scale-90 ${styles[status] || "bg-slate-500/10 text-slate-500"}`}
...
const UserManagement = () => {
  ...
  // Lọc kép: Theo vai trò và Tên đăng nhập (username)
  ...
  const toggleStatus = (e, id) => {
    e.stopPropagation(); // Ngăn sự kiện nhấn vào hàng (mở profile)
    setUsers(users.map(u => u.userId === id 
      ? { ...u, status: u.status === 'ACTIVE' ? 'BANNED' : 'ACTIVE' } 
      : u
    ));
    // Cập nhật selectedUser nếu đang hiển thị profile của người đó
                ...
                {filteredUsers.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <StatusBadge status={user.status} onClick={(e) => toggleStatus(e, user.userId)} />
                    </td>
                    <td className="px-6 py-4">
          ...
          {filteredUsers.length === 0 && (
            <div className="p-16 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
               ...
               {searchUsername && (
                 <button 
                   onClick={() => setSearchUsername('')}
                   className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 transition-colors"
                 >
                   Xóa tìm kiếm
                 </button>
               )}
        ...
        {selectedUser && (
          <div className="lg:col-span-5 bg-white dark:bg-[#0b1120] border border-slate-200 dark:border-white/5 rounded-3xl p-8 shadow-xl animate-in slide-in-from-right-4 duration-300 relative h-fit sticky top-24">
            <button 
              ...
              className="absolute top-6 right-6 p-2 text-slate-400 hover:text-red-500 transition-all rounded-lg hover:bg-red-500/5"
            >
              <X size={20} />
            </button>
            ...
            <div className="flex flex-col items-center text-center mb-8">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
                ...
                <div className="w-1 h-1 bg-slate-300 rounded-full"></div>
                <StatusBadge status={selectedUser.status} onClick={(e) => toggleStatus(e, selectedUser.userId)} />
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <button className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-[1.02] transition-transform active:scale-95 shadow-xl shadow-slate-500/10">
                Gửi thông báo riêng
              </button>
            </div>
          </div>
